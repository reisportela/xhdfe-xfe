...
#
# The committed fixture was generated by a reference build compiled with
# -march=native. This is a SAME-BUILD regression check and is skipped on CI,
# where the package is compiled with a portable -march and the iterative
# solvers drift slightly (numerical, not econometric). Cross-build correctness
# is covered by test-interface.R (lm/fixest agreement) and test-regressions.R,
# which do run on CI. Set XHDFE_RUN_PARITY=1 to force this suite anywhere.
if (Sys.getenv("XHDFE_RUN_PARITY") != "1") testthat::skip_on_ci()
