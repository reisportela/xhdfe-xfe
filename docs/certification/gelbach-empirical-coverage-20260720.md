...
no manifesto local arquivado no histórico Git. A admissão no
...
Um novo oracle determinístico em `tests/validation/VALIDATE_GELBACH.py` inclui uma focal, um
...
- `tests/validation/VALIDATE_GELBACH.py`: **PASS**, incluindo focal inerte, shares, contraste,
  common-controls/LSDV e oracle externo absorbed-target;
- `tests/validation/VALIDATE_GELBACH_FRONTENDS.py`: **PASS**, paridade standard e absorbed entre
