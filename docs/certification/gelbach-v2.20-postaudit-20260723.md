...
- `tests/validation/VALIDATE_GELBACH.py`: a `4e-9` straddle case warns and marks; a `2e-2`
  focal is silent; the absorbed-side ratio remains classified exactly as
  before.
- `tests/validation/VALIDATE_GELBACH_ADVERSARIAL.py`: default warning contract and kill-switch
...
independent grid in `tests/validation/VALIDATE_GELBACH.py` is the authoritative final oracle
test.
...
`tests/validation/VALIDATE_GELBACH.py` exercises:
...
- `tests/validation/VALIDATE_GELBACH_ADVERSARIAL.py`: saturated guard and post-error healthy
  reuse pass.
...
- Frontend validator description:
  `tests/validation/VALIDATE_GELBACH_FRONTENDS.py:1-10`.
- Release summary:
  `docs/releases/RELEASE_NOTES_2.20.0.20260723.md`.
...
`tests/validation/VALIDATE_GELBACH_HELP.py` passes all 57 checks. The historical wording inside
...
| Validation | Result |
|---|---|
| `python3 tests/validation/VALIDATE_GELBACH.py --module-dir build --external-absorbed-oracle require --stata stata-mp` | **PASS: all checks** |
...
| `python3 tests/validation/VALIDATE_GELBACH_FRONTENDS.py ... --module-dir build` | **PASS: all CPU frontend and example checks** |
...
| `python3 tests/validation/VALIDATE_GELBACH_ADVERSARIAL.py --module-dir build` | **PASS: all checks** |
| `python3 tests/validation/VALIDATE_GELBACH_HELP.py` | **PASS: 57 checks** |
...
| `python3 tests/validation/VALIDATE_AKM_KSS.py --module-dir build` | **PASS: all checks; expected CPU-build GPU skip** |
...
- `tests/validation/VALIDATE_GELBACH_FRONTENDS.py`
- `tests/validation/VALIDATE_GELBACH_ADVERSARIAL.py`
- `tests/validation/VALIDATE_GELBACH_HELP.py`
