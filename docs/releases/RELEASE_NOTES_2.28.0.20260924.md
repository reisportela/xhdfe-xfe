# xhdfe 2.28.0.20260924

Release date: 24 September 2026. The xhdfe command version is 2.28.0;
...
The local campaign records frozen sources and native binaries. Release artifacts
are rebuilt in CI and checked independently before publication.
...
Close the Stata session that loaded the old plugin. In a fresh session, reinstall
with `net install ..., replace` and rerun the estimation.
