...
struct LeaveOutSetResult {
    ...
    int threads_requested = 0;     //!< Caller request; 0 selects the automatic policy.
    int threads_effective = 1;     //!< Request after the runtime-capacity limit.
    int threads_used = 1;          //!< Largest OpenMP team observed doing sample-construction work.
    int parallel_workers_active = 1; //!< Largest number of workers observed executing loop iterations.
    int thread_capacity = 1;       //!< Runtime-visible processor/OpenMP capacity.
    bool openmp_enabled = false;
    int thread_limit_code = 0;     //!< 0 none, 1 runtime capacity, 4 OpenMP unavailable.
...
struct AkmKssResult {
    ...
    bool gpu_requested = false;      //!< CUDA was requested for the two-way solver.
    bool gpu_used = false;           //!< CUDA solver actually used for the PCG solves.
    int gpu_status_code = 0;         //!< Solver status: 0 not requested, 1 used, 2 unavailable, 6 not applicable.
    std::string gpu_backend = "cpu"; //!< Backend that actually solved the two-way systems.
    std::string gpu_status = "not_requested";
    bool solver_direct = true;       //!< Direct Cholesky vs PCG actually used.
    int threads_requested = 0;       //!< Caller request; 0 selects automatic phase policies.
    int threads_effective = 1;       //!< Command budget after the runtime-capacity limit.
    int solver_threads_effective = 1; //!< Solver team after automatic work heuristics; explicit requests are not heuristically capped.
    int fwl_threads_effective = 0;   //!< FWL budget after capacity/automatic policy (0 when no controls).
    int fwl_threads_used = 0;        //!< Largest FWL team observed doing real work (0 when no controls).
    int fwl_parallel_workers_active = 0; //!< FWL workers observed executing iterations (0 when no controls).
    int solver_threads_used = 0;     //!< Largest solver team observed doing real work (0 if the solver phase did not run).
    int solver_parallel_workers_active = 0; //!< Solver workers observed executing iterations (0 if the phase did not run).
    bool fwl_gpu_attempted = false;  //!< CUDA entered the FWL absorber (currently false: FWL is CPU-reference only).
    bool fwl_gpu_used = false;       //!< The accepted FWL result came from CUDA (currently false).
    bool fwl_gpu_fallback = false;   //!< Reserved for a future certified CUDA-to-CPU retry path.
    int fwl_gpu_status_code = 6;     //!< 6 no controls; 8 certified CPU-reference policy.
    std::string fwl_gpu_status = "not_applicable";
    int fwl_iterations = 0;          //!< Iterations of the accepted FWL result (0 when no controls).
    double fwl_abs_residual_rel = 0.0; //!< Independent backward-error certificate for the accepted FWL result.
    bool fwl_precision_certified = true; //!< The accepted FWL transform passed its declared precision certificate.
    int threads_used = 1;            //!< Largest OpenMP team observed doing work in any AKM/KSS phase.
    int parallel_workers_active = 1; //!< Largest number of workers observed executing loop iterations.
    int thread_capacity = 1;         //!< Runtime-visible processor/OpenMP capacity.
    bool openmp_enabled = false;
    int thread_limit_code = 0;       //!< 0 none, 1 runtime capacity, 4 OpenMP unavailable.
...
struct GelbachResult {
    ...
    int threads_requested = 0;       //!< Caller request; 0 selects automatic phase policies.
    int threads_effective = 1;       //!< Command budget after the runtime-capacity limit.
    int recovery_threads_effective = 1; //!< FE-recovery/covariance team after automatic heuristics; explicit requests are not heuristically capped.
    int fullfit_threads_used = 0;     //!< Largest team observed in the full-model fit(s); 0 if absent.
    int fullfit_parallel_workers_active = 0; //!< Full-model workers observed executing iterations; 0 if absent.
    int recovery_threads_used = 0;    //!< Largest team observed in FE recovery/certification; 0 if absent.
    int recovery_parallel_workers_active = 0; //!< Recovery workers observed executing iterations; 0 if absent.
    int covariance_threads_used = 0;  //!< Largest team observed in base/auxiliary/covariance work; 0 if absent.
    int covariance_parallel_workers_active = 0; //!< Covariance workers observed executing iterations; 0 if absent.
    int threads_used = 1;            //!< Largest OpenMP team observed doing real work.
    int parallel_workers_active = 1; //!< Largest number of workers observed executing loop iterations.
    int thread_capacity = 1;         //!< Runtime-visible processor/OpenMP capacity.
    bool openmp_enabled = false;
    int thread_limit_code = 0;       //!< 0 none, 1 runtime capacity, 4 OpenMP unavailable.
