...
ParsedDofAdjustments parse_dofadjustments(std::optional<std::string> raw_opt) {
    ...
    const bool token_free = std::all_of(
        raw_opt->begin(), raw_opt->end(), [](unsigned char c) {
...
void maybe_store_dof_table(const std::optional<std::string>& name,
    ...
    // Optional columns used by the ado for heterogeneous-slope display:
    // 6 = distinct base FE levels; 7 = redundant base-intercept categories.
    const int have_rows = SF_row(const_cast<char*>(name->c_str()));
    const int have_cols = SF_col(const_cast<char*>(name->c_str()));
    if (have_rows != rows || (have_cols != 5 && have_cols != 6 && have_cols != 7)) {
        throw_with_prefix(
            "xhdfe plugin: ",
            "dof_table has wrong dimensions (have " + std::to_string(have_rows) + "x" +
                std::to_string(have_cols) + ", want " + std::to_string(rows) + "x5, x6, or x7)");
    }
    ...
    for (int i = 0; i < rows; ++i) {
        ...
        if (have_cols >= 7) {
            rc7 = SF_mat_store(const_cast<char*>(name->c_str()), i + 1, 7,
                               get_val(res.fe_base_redundant, i));
        }
        if (rc1 || rc2 || rc3 || rc4 || rc5 || rc6 || rc7) {
...
ST_retcode run_gelbach(const ParsedArgs& args) {
    ...
    if (auto val = args.get_optional("absorbed_x1")) {
        std::string buf;
        for (const char c : *val) {
            if (c == ',') {
                if (!buf.empty()) {
                    options.absorbed_x1.push_back(std::stoi(buf));
                    buf.clear();
                }
            } else {
                buf.push_back(c);
    ...
    for (int c = 0; c < p; ++c) {
        ...
        SF_mat_store(const_cast<char*>(absmat.c_str()), 1, c + 1,
                     static_cast<double>(res.x1_absorbed[c]));
    }
...
STDLL stata_call(int argc, char* argv[]) {
    ...
    try {
        ...
        if (msg.find("no observations remain for estimation") != std::string::npos) {
            return static_cast<ST_retcode>(2001);  // insufficient observations
        }
